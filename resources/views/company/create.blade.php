    ...
    <div class="overflow-x-auto p-6">
        <form action="{{ route('company.store', ['redirecttolist' => request('redirecttolist')]) }}" method="POST"
            ...
            <div class="mb-4 p-6 bg-gray-50 border rounded-lg shadow-md">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="owner_email" id="owner_email" value="{{ old('owner_email') }}"
                        class="w-full px-3 py-2 rounded-lg focus:outline-none border {{ $errors->has('owner_email') ? 'border-red-500 focus:ring-2 focus:ring-blue-500' : 'border-gray-300 focus:ring-2 focus:ring-blue-500' }}"
                        required>
                    @error('owner_email')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div class="flex justify-end space-x-2">
                <a href="{{ route('company.index') }}" class="text-gray-600 hover:text-gray-900 px-4 py-2">Cancel</a>
                <button type="submit"
